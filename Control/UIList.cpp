...
//
//
UI_IMPLEMENT_DYNCREATE(CListUI);
UI_IMPLEMENT_DYNCREATE(CListHeaderUI);
UI_IMPLEMENT_DYNCREATE(CListHeaderItemUI);
UI_IMPLEMENT_DYNCREATE(CListLabelElementUI);
UI_IMPLEMENT_DYNCREATE(CListTextElementUI);
UI_IMPLEMENT_DYNCREATE(CListContainerElementUI);
...
void CListUI::SetPos(RECT rc, bool bNeedInvalidate /*= true*/)
{
...
void CListUI::Move(SIZE szOffset, bool bNeedInvalidate /*= true*/)
{
...
void CListUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        ...
        {
            CVerticalLayoutUI::EventHandler(event);
        }
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    if(m_pManager != NULL)
    {
        m_pManager->SendNotify(this, UINOTIFY_ITEM_LCLICK, m_iCurSel, iOldSel);
    }
...
void CListBodyUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pOwner != NULL)
...
{
    if(_tcscmp(pstrName, _T("ListHeader")) == 0)
    {
        return this;
    }
...
SIZE CListHeaderUI::EstimateSize(SIZE szAvailable)
{
    ...
    if(cXY.cy == 0 && m_pManager != NULL)
    {
        ...
        int nMin = GetResEngine()->GetDefaultFont()->GetTextMetric().tmHeight + 6;
...
{
    if(_tcscmp(pstrName, _T("ListHeaderItem")) == 0)
    {
        return this;
    }
...
void CListHeaderItemUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pParent != NULL)
    ...
    if(event.dwType == UIEVENT_LBUTTONDOWN || event.dwType == UIEVENT_LDBLCLICK)
    {
        ...
        else
        {
            ...
            m_pManager->SendNotify(this, UINOTIFY_HEADER_CLICK);
            Invalidate();
        }
...
SIZE CListHeaderItemUI::EstimateSize(SIZE szAvailable)
{
    if(m_cxyFixed.cy == 0)
    {
        return CDuiSize(m_cxyFixed.cx, GetResEngine()->GetDefaultFont()->GetTextMetric().tmHeight + 14);
    }
...
void CListHeaderItemUI::PaintStatusImage(HDC hDC)
{
    ...
    {
        if(m_sPushedImage.empty() && !m_sNormalImage.empty())
        {
            DrawImage(hDC, (LPCTSTR)m_sNormalImage.c_str());
        }
    ...
    {
        if(m_sHotImage.empty() && !m_sNormalImage.empty())
        {
            DrawImage(hDC, (LPCTSTR)m_sNormalImage.c_str());
        }
    ...
    {
        if(m_sFocusedImage.empty() && !m_sNormalImage.empty())
        {
            DrawImage(hDC, (LPCTSTR)m_sNormalImage.c_str());
        }
    ...
    if(!m_sSepImage.empty())
    {
        ...
        m_sSepImageModify = CDuiStringOperation::format(_T("dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom);

        if(!DrawImage(hDC, (LPCTSTR)m_sSepImage.c_str(), (LPCTSTR)m_sSepImageModify.c_str()))
        {
            m_sSepImage.clear();
        }
    }
...
bool CListElementUI::Activate()
{
    ...
    if(m_pManager != NULL)
    {
        m_pManager->SendNotify(this, UINOTIFY_ITEM_ACTIVATE);
    }
...
void CListElementUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pOwner != NULL)
    ...
    if(event.dwType == UIEVENT_KEYDOWN && IsEnabled())
    {
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT & rcItem)
{
    ...
    if(!IsEnabled())
    {
        ...
        {
            if(!DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage.c_str()))
            {
    ...
    if(IsSelected())
    {
        ...
        {
            if(!DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage.c_str()))
            {
    ...
    if((m_uButtonState & UISTATE_Hover) != 0)
    {
        ...
        {
            if(!DrawImage(hDC, (LPCTSTR)pInfo->sHotImage.c_str()))
            {
    ...
    if(!m_sBkImage.empty())
    {
        if(!pInfo->bAlternateBk || m_iIndex % 2 == 0)
        {
            if(!DrawImage(hDC, (LPCTSTR)m_sBkImage.c_str()))
            {
                m_sBkImage.clear();
            }
        }
    ...
    if(m_sBkImage.empty())
    {
        ...
        {
            if(!DrawImage(hDC, (LPCTSTR)pInfo->sBkImage.c_str()))
            {
...
void CListLabelElementUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pOwner != NULL)
        ...
        {
            CListElementUI::EventHandler(event);
        }

        return;
    }

    if(event.dwType == UIEVENT_LBUTTONDOWN || event.dwType == UIEVENT_RBUTTONDOWN)
    {
        if(IsEnabled())
        {
            m_pManager->SendNotify(this, UINOTIFY_ITEM_LCLICK);
            Select();
            Invalidate();
        }
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if(cXY.cy == 0 && m_pManager != NULL)
    {
        cXY.cy = GetResEngine()->GetFont(pInfo->strFontID.c_str())->GetTextMetric().tmHeight + 8;
    ...
    if(cXY.cx == 0 && m_pManager != NULL)
    {
        ...
        {
            int nLinks = 0;
            CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), 0, NULL, NULL, nLinks, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
        else
        {
            CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), 0, pInfo->strFontID.c_str(), DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT & rcItem)
{
    ...
    if(pInfo->bShowHtml)
        ...
        CRenderEngine::DrawText(hDC, m_pManager, rcText, m_sText.c_str(), iTextColor, \
                                pInfo->strFontID.c_str(), DT_SINGLELINE | pInfo->uTextStyle);
}
...
void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText)
{
    ...
    if((pText == NULL && pstrText == NULL) || (pText && pText->compare(pstrText) == 0))
    {
        return;
    }
...
void CListTextElementUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pOwner != NULL)
        ...
        {
            CListLabelElementUI::EventHandler(event);
        }
    ...
    if(event.dwType == UIEVENT_LBUTTONUP && IsEnabled())
    {
        for(int i = 0; i < m_nLinks; i++)
        {
            if(::PtInRect(&m_rcLinks[i], event.ptMouse))
            {
                m_pManager->SendNotify(this, UINOTIFY_ITEM_LCLICK, i);
                return;
            }
        }
    }

    if(m_nLinks > 0 && event.dwType == UIEVENT_MOUSEMOVE)
    {
        int nHoverLink = -1;
    ...
    if(m_nLinks > 0 && event.dwType == UIEVENT_MOUSELEAVE)
    {
        if(m_nHoverLink != -1)
        {
            Invalidate();
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT & rcItem)
{
    ...
    for(int i = 0; i < pInfo->nColumns; i++)
    {
        ...
        DWORD dwAlign = pInfo->uTextStyle;

        if(pCallback)
        {
            strText = pCallback->GetItemText(this, m_iIndex, i);
            dwAlign = pCallback->GetTextAlign(this, m_iIndex, i, dwAlign);
        }
        else
        {
            strText = GetText(i);
        ...
        if(pInfo->bShowHtml)
            ...
            CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.c_str(), iTextColor, \
                                    pInfo->strFontID.c_str(), DT_SINGLELINE | dwAlign);
...
LPVOID CListContainerElementUI::GetInterface(LPCTSTR pstrName)
{
    ...
    if(_tcscmp(pstrName, _T("ListContainerElement")) == 0)
    {
...
void CListContainerElementUI::EventHandler(TEventUI & event)
{
    if(!IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND)
    {
        if(m_pOwner != NULL)
    ...
    if(event.dwType == UIEVENT_LDBLCLICK)
    {
        if(IsEnabled())
        {
            m_pManager->SendNotify(this, UINOTIFY_ITEM_LDBCLICK);
            Activate();
            Invalidate();
        }
    ...
    if(event.dwType == UIEVENT_RBUTTONDOWN)
    {
        if(IsEnabled())
        {
            m_pManager->SendNotify(this, UINOTIFY_ITEM_RCLICK);
            Select();
            Invalidate();
        }
    ...
    if(event.dwType == UIEVENT_MOUSEENTER)
    {
        if(IsEnabled())
        {
            ...
            if(m_pManager)
            {
                m_pManager->SendNotify(this, UINOTIFY_ITEM_Enter);
            }
    ...
    if(event.dwType == UIEVENT_MOUSELEAVE)
    {
        if((m_uButtonState & UISTATE_Hover) != 0)
        {
            ...
            if(m_pManager)
            {
                m_pManager->SendNotify(this, UINOTIFY_ITEM_Leave);
            }

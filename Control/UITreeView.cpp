...
namespace DuiLib
{
	UI_IMPLEMENT_DYNCREATE(CTreeNodeUI);
	UI_IMPLEMENT_DYNCREATE(CTreeViewUI);
	...
	// 函数说明:
	//************************************
	void CTreeNodeUI::EventHandler( TEventUI& event )
	{
		if( !IsMouseEnabled() && event.dwType > UIEVENT__MOUSEBEGIN && event.dwType < UIEVENT__MOUSEEND ) {
			...
			else CContainerUI::EventHandler(event);
			return;
		}

		CListContainerElementUI::EventHandler(event);
	...
	// 函数说明: 
	//************************************
	bool CTreeViewUI::Notify(TNotifyUI *pMsg)
	{
		return false;
	}
	...
	bool CTreeViewUI::OnCheckBoxChanged( void* param )
	{
		...
		if(pMsg->dwType == UINOTIFY_SELCHANGE)
		{
	...
	{
		TNotifyUI* pMsg = (TNotifyUI*)param;
		if(pMsg->dwType == UINOTIFY_ITEM_LDBCLICK)
		{

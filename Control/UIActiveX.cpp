...
class CActiveXCtrl;
UI_IMPLEMENT_DYNCREATE(CActiveXUI);
...
public:
    ...
    STDMETHOD(GetWindow)(HWND* phwnd)
    {
        ...
        *phwnd = m_pOwner->GetManager()->GetHWND();
        return S_OK;
    }
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)        
{
    ...
    HWND hWndFrame = m_pOwner->GetManager()->GetHWND();
    HRESULT Hr = E_FAIL;
    if( (dwFlags & ACTIVATE_WINDOWLESS) != 0 ) {
        ...
        m_pOwner->GetManager()->AddWindowMessageFilter(m_pOwner);
    }
    if( FAILED(Hr) ) {
        m_bWindowless = false;
        Hr = CreateActiveXWnd();
...
STDMETHODIMP CActiveXCtrl::GetWindowContext(IOleInPlaceFrame** ppFrame, IOleInPlaceUIWindow** ppDoc, LPRECT lprcPosRect, LPRECT lprcClipRect, LPOLEINPLACEFRAMEINFO lpFrameInfo)
{
    ...
    *ppDoc = NULL;
    ...
    lpFrameInfo->hwndFrame = m_pOwner->GetManager()->GetHWND();
...
HRESULT CActiveXCtrl::CreateActiveXWnd()
{
    ...
    m_pOwner->m_hwndHost = m_pWindow->Init(this, m_pOwner->GetManager()->GetHWND());
    return S_OK;
}
...
LRESULT CActiveXWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( !bHandled ) return CWindowWnd::WindowProc(uMsg, wParam, lParam);
    return lRes;
}
...
void CActiveXUI::Move(SIZE szOffset, bool bNeedInvalidate /*= true*/)
{
...
LRESULT CActiveXUI::MessageFilter(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if( !m_sModuleName.empty() ) {
        HMODULE hModule = ::LoadLibrary((LPCTSTR)m_sModuleName.c_str());
        if( hModule != NULL ) {
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify((CControlUI*)this, UINOTIFY_ACTIVEX_SHOW, 0, 0, false);
    if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
        Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pOleClientSite, 0, m_pManager->GetHWND(), &m_rcItem);
        //::RedrawWindow(m_pManager->GetHWND(), &m_rcItem, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE | RDW_INTERNALPAINT | RDW_FRAME);
    }
    IObjectWithSite* pSite = NULL;
    m_pUnk->QueryInterface(IID_IObjectWithSite, (LPVOID*) &pSite);
    if( pSite != NULL ) {

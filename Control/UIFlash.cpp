...
namespace DuiLib
{
	UI_IMPLEMENT_DYNCREATE(CFlashUI);
	...
	HRESULT CFlashUI::FlashCall( _bstr_t request )
	{
		if (m_pFlashEventHandler)
		{ 
			return m_pFlashEventHandler->FlashCall(request,m_pFlash);
		}
		return S_OK;
	}
	...
	//调用Flash中的方法
	void CFlashUI::CallFlashFunction(LPCTSTR pstrFuncName, LPCTSTR pstrPara)
	{
		if(m_pFlash)
		{
			CDuiString strIn,strOut;
			if(_tcscmp(pstrPara, _T("")) == 0)
			{
				strIn = CDuiStringOperation::format(_T("<invoke name='%s' returntype='xml'><arguments></arguments></invoke>"),pstrFuncName);	
			}
			else
			{
				strIn = CDuiStringOperation::format(_T("<invoke name='%s' returntype='xml'><arguments><string>%s</string></arguments></invoke>"),pstrFuncName,pstrPara);
			}
			BSTR bstrIn = SysAllocString(strIn.c_str());
			BSTR bstrOut = SysAllocString(strOut.c_str());
			m_pFlash->CallFunction(bstrIn,&bstrOut);

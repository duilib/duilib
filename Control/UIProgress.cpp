...
namespace DuiLib
{
	UI_IMPLEMENT_DYNCREATE(CProgressUI);
	...
	{
		if( m_sForeImage.compare(pStrImage) ==  0) return;
	...
	void CProgressUI::PaintStatusImage(HDC hDC)
	{
		...
		if( !m_sForeImage.empty() ) 
		{
			...
			if (m_bStretchForeImage)
				m_sForeImageModify = CDuiStringOperation::format(_T("dest='%d,%d,%d,%d'"), rc.left, rc.top, rc.right, rc.bottom);
			else
				m_sForeImageModify = CDuiStringOperation::format(_T("dest='%d,%d,%d,%d' source='%d,%d,%d,%d'")
				, rc.left, rc.top, rc.right, rc.bottom
				, rc.left, rc.top, rc.right, rc.bottom);

			if( !DrawImage(hDC, (LPCTSTR)m_sForeImage.c_str(), (LPCTSTR)m_sForeImageModify.c_str()) )
				m_sForeImage.clear();
			else 
				return;
		}

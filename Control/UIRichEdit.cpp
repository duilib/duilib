...
HRESULT InitDefaultCharFormat(CRichEditUI *re, CHARFORMAT2W *pcf, HFONT hfont)
{
    ...
    if(!hfont)
    {
        hfont = GetResEngine()->GetFont(re->GetFontID())->GetFont();
    }
...
{
    if(_tcscmp(pstrName, _T("RichEdit")) == 0)
    {
...
void CRichEditUI::SetFontID(LPCTSTR lpszFontID)
{
	m_strFontID = lpszFontID;

	if(m_pTwh)
	{
		m_pTwh->SetFont(GetResEngine()->GetFont(GetFontID())->GetFont());
	}
}
    ...
    if(m_pTwh)
    {
        if(msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN)
        {
            if(!m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn))
            {
                if(m_pManager != NULL)
                {
                    m_pManager->SendNotify((CControlUI *)this, UINOTIFY_RICHEDIT_RETURN);
                }
...
void CRichEditUI::EventHandler(TEventUI & event)
{
    ...
    if(event.dwType == UIEVENT_SETFOCUS)
    {
        ...
        {
            m_pTwh->OnTxInPlaceActivate(NULL);
            hr = m_pTwh->GetTextServices()->TxSendMessage(WM_SETFOCUS, 0, 0, 0);
        }

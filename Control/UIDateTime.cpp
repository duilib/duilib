...
namespace DuiLib
{
	UI_IMPLEMENT_DYNCREATE(CDateTimeUI);
...
#define DT_KEEP   3
	...
	void CDateTimeWnd::Init(CDateTimeUI* pOwner)
	{
		...
		if (m_hWnd == NULL)
		{
			...
			SetWindowFont(m_hWnd, GetResEngine()->GetFont(m_pOwner->GetFontID())->GetFont(), TRUE);
		}

		if (m_pOwner->GetText() != NULL )
	...
	void CDateTimeUI::UpdateText()
	{
		...
		{
			CDuiString sText = CDuiStringOperation::format(_T("%4d-%02d-%02d"),
				m_sysTime.wYear, m_sysTime.wMonth, m_sysTime.wDay, m_sysTime.wHour, m_sysTime.wMinute);
			SetText(sText.c_str());
		}
	}

	void CDateTimeUI::EventHandler(TEventUI& event)
	{
		...
		if( event.dwType == UIEVENT_SETFOCUS && IsEnabled() ) 
		{
			if( m_pWindow ) return;

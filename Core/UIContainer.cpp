...
{
    if(_tcscmp(pstrName, _T("IContainer")) == 0)
    {
        return static_cast<IContainerUI *>(this);
    }
    else if(_tcscmp(pstrName, _T("Container")) == 0)
    {
...
void CContainerUI::VisibleState(bool bVisible)
{
}

void CContainerUI::EventHandler(TEventUI & event)
{
...
void CContainerUI::LineUp()
{
    int cyLine = 8;
    //if( m_pManager )
    cyLine = GetResEngine()->GetDefaultFont()->GetTextMetric().tmHeight + 8;
...
void CContainerUI::EnableScrollBar(bool bEnableVertical, bool bEnableHorizontal)
{
    if(bEnableVertical && !m_pVerticalScrollBar)
    {
        ...
        if(m_pManager)
        {
            GetResEngine()->ApplyResTemplate(m_pVerticalScrollBar, _T("VScrollBar"));
        }
    }
    ...
    if(bEnableHorizontal && !m_pHorizontalScrollBar)
    {
        ...
        if(m_pManager)
        {
            GetResEngine()->ApplyResTemplate(m_pHorizontalScrollBar, _T("HScrollBar"));
        }
    }
...
void CContainerUI::SetPos(RECT rc, bool bNeedInvalidate /*= true*/)
{
...
void CContainerUI::Move(SIZE szOffset, bool bNeedInvalidate /*= true*/)
{
...
void CContainerUI::SetManager(CWindowUI *pManager, CControlUI *pParent, bool bInit)
{
    for(int it = 0; it < m_items.GetSize(); it++)
    {
...
void CContainerUI::DoPaint(HDC hDC, const RECT & rcPaint)
{
    ...
    CStdPtrArray temp(m_items);
    temp.Sort(ZOrderCompare);
...
{
    CControlUI::SetNotifyFilter(pNotifyFilter);

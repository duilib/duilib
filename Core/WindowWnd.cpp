...
HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR lpszWindowName, DWORD dwStyle, DWORD dwExStyle, int x /*= CW_USEDEFAULT*/, int y /*= CW_USEDEFAULT*/, int cx /*= CW_USEDEFAULT*/, int cy /*= CW_USEDEFAULT*/)
{
	...
	m_hWnd = ::CreateWindowEx(dwExStyle, GetWindowClassName(), lpszWindowName, dwStyle, x, y, cx, cy,
		hwndParent, NULL, GetUIEngine()->GetInstanceHandle(), this);
...
bool CWindowWnd::RegisterWindowClass()
{
	...
	wc.hInstance = GetUIEngine()->GetInstanceHandle();
...
bool CWindowWnd::ShowWindow(int nCmdShow /*= SW_SHOW*/)
{
	ASSERT(::IsWindow(m_hWnd));
	if ( ::IsWindow(m_hWnd))
	{
...
bool CWindowWnd::IsMaximized()
{
	return ::IsZoomed(m_hWnd) == 0;
}

bool CWindowWnd::IsMinimized()
{
...
void CWindowWnd::GetClientRect(LPRECT rc)
{
...
bool CWindowWnd::RegisterSuperclass()
{
	...
	{
		if( !::GetClassInfoEx(GetUIEngine()->GetInstanceHandle(), GetSuperClassName(), &wc) )
		{
...
BOOL CWindowWnd::MoveWindow(int x,int y,int width /*= -1*/, int height /*= -1*/,BOOL bRepaint/*= FALSE*/)
{
    ...
    return ::SetWindowPos(this->m_hWnd, NULL, x, y, 0,0,SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE);

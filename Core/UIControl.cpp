...
CWindowUI *CControlUI::GetManager() const
{
    return m_pManager;
}

void CControlUI::SetManager(CWindowUI *pManager, CControlUI *pParent, bool bInit)
{
    m_pManager = pManager;
    m_pParent = pParent;
...
{
    if(m_sBkImage.compare(pStrImage) == 0)
    {
        return;
    }
...
void CControlUI::SetPos(RECT rc, bool bNeedInvalidate/* = true*/)
{
    if(rc.right < rc.left)
    {
        rc.right = rc.left;
    ...
    if(!m_bSetPos)
    {
        m_bSetPos = true;   // 防止循环调用SetPos接口
        SendEvent(UIEVENT_WINDOWSIZE);
        m_bSetPos = false;
    }
...
void CControlUI::SetPos(int x, int y, bool bNeedInvalidate /*= true*/)
{
    RECT rc = {x, y, x + m_cxyFixed.cx, y + m_cxyFixed.cy};
    SetPos(rc,bNeedInvalidate);
}

void CControlUI::Move(SIZE szOffset, bool bNeedInvalidate /*= true*/)
{
    CDuiRect invalidateRc = m_rcItem;
...
DuiLib::TPercentInfo CControlUI::GetFloatPercent() const
{
...
{
    m_sToolTip = pstrText;
    CDuiStringOperation::replace(m_sToolTip, _T("<n>"), _T("\r\n"));
}
...
void CControlUI::EventHandler(TEventUI & event)
{
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    {
        RECT rcPos = { 0 };
		CDuiCodeOperation::StringToRect(pstrValue,&rcPos);
        SIZE szXY = {rcPos.left, rcPos.top};
        SetFixedXY(szXY);
		if ( rcPos.right != 0 || rcPos.bottom != 0 )
		{
    ...
    else if(_tcscmp(pstrName, _T("bordersize")) == 0)
    {
        ...
        {
            RECT rcPadding = { 0 };
            CDuiCodeOperation::StringToRect(pstrValue, &rcPadding);
            SetBorderSize(rcPadding);
        }
    }
    ...
    {
        SetMouseEnabled(_tcscmp(pstrValue, _T("true")) == 0);
    }
    else if(_tcscmp(pstrName, _T("keyboard")) == 0)
    {
        SetKeyboardEnabled(_tcscmp(pstrValue, _T("true")) == 0);
    }
    ...
    {
        SetContextMenuUsed(_tcscmp(pstrValue, _T("true")) == 0);
    }
}
...
void CControlUI::SetAlpha(DWORD dwAlpha)
{
	if ( m_nAlpha == dwAlpha)
		return;
	m_nAlpha = dwAlpha;
...
bool CControlUI::SendEvent(UIEVENT dwType)
{
    TEventUI event;
    event.dwType = dwType;
    event.pSender = this;
...
bool CControlUI::SendEvent(TEventUI *pEventUI)
{
    if(this->OnEvent.respondsToID(pEventUI->dwType))
    {
        return this->OnEvent.Invoke(pEventUI, pEventUI->dwType);
    }

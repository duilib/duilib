...
bool TiXmlDocument::LoadFile( FILE* file, TiXmlEncoding encoding )
{
	...
	*/
	...
	bool bRet = LoadXmlString(buf, length, encoding);
	delete [] buf;
	return bRet;
...
bool TiXmlDocument::LoadXmlString( const char *xmlDoc, int length, TiXmlEncoding encoding /*= TIXML_DEFAULT_ENCODING*/ )
{
	const char *buf = xmlDoc;
	const char* lastPos = buf;
	const char* p = buf;

	if(encoding == TIXML_DEFAULT_ENCODING)
	{
		encoding = FindEncoding(xmlDoc, length);
	}
...
TiXmlEncoding TiXmlDocument::FindEncoding( const char *xmlDoc , int docLen)
{
	BYTE *pDoc = (BYTE*)(xmlDoc);
	if(docLen > 3 && pDoc[0]==0xEF &&pDoc[1]==0xBB && pDoc[2]==0xBF)
	{
		return TIXML_ENCODING_UTF8;
	}
	else if(docLen > 4)
	{
		WORD val = *(WORD*)xmlDoc;
		DWORD dwVal = *(DWORD*)xmlDoc;
		if(val == 0xFFFE || val == 0xFEFF)
		{
			return TIXML_ENCODING_LEGACY;
		}
		else if(dwVal == 0xFEFE || dwVal == 0xFFFE0000)
		{
			return TIXML_ENCODING_LEGACY;
		}
		
	}
	return TIXML_DEFAULT_ENCODING;
}

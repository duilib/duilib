...
namespace DuiLib
{
	...
	void CFlashUI::CallFlashFunction(LPCTSTR pstrFuncName, LPCTSTR pstrPara)
	{
		if(m_pFlash)
		{
			...
			{
				strIn.Format(_T("<invoke name='%s' returntype='xml'><arguments></arguments></invoke>"),pstrFuncName);	
			}
			else
			{
				strIn.Format(_T("<invoke name='%s' returntype='xml'><arguments><string>%s</string></arguments></invoke>"),pstrFuncName,pstrPara);
			}
			BSTR bstrIn = SysAllocString(strIn);
			BSTR bstrOut = SysAllocString(strOut);

...
HRESULT InitDefaultCharFormat(CRichEditUI* re, CHARFORMAT2W* pcf, HFONT hfont) 
{
	...
	if( !hfont )
		hfont = CResourceManager::GetInstance()->GetFont(re->GetFontName())->GetFont();
...
UI_IMPLEMENT_DYNCREATE(CRichEditUI);
...
CRichEditUI::~CRichEditUI()
{
	if( m_pTwh ) {
		m_pTwh->Release();
		m_pManager->RemoveWindowMessageFilter(this);
	}
}
...
void CRichEditUI::SetFontName(LPCTSTR lpszFontName)
{
	...
	if( m_pTwh )
	{
		m_pTwh->SetFont(CUIEngine::GetInstance()->GetFont(lpszFontName));
	}
}
...
LPCTSTR CRichEditUI::GetText() const
{
...
void CRichEditUI::DoInit()
{
	...
	if( m_pTwh ) {
		...
		m_pManager->AddWindowMessageFilter(this);
	}
	...
	if( m_pTwh ) {
		if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
			if( !m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn) ) {
				if( m_pManager != NULL ) m_pManager->SendNotify((CControlUI*)this, UINOTIFY_RICHEDIT_RETURN);
				return S_OK;
			}
		}
	...
	case EN_DRAGDROPDONE:   
		{
			...
			{   
				LONG nId =  GetWindowLong(this->GetManager()->GetHWND(), GWL_ID);   
				NMHDR  *phdr = (NMHDR *)pv;   
				phdr->hwndFrom = this->GetManager()->GetHWND();   
				phdr->idFrom = nId;   
				phdr->code = iNotify;  

				if(SendMessage(this->GetManager()->GetHWND(), WM_NOTIFY, (WPARAM) nId, (LPARAM) pv))   
				{   
					//hr = S_FALSE;   
				}   
			}    
...
bool CRichEditUI::EventHandler(TEventUI& event)
{
	...
	if( event.dwType > UIEVENT__KEYBEGIN && event.dwType < UIEVENT__KEYEND )
	{
		return true;
	}
...
void CRichEditUI::SetPosition(LPCRECT lpRect)
{
	CControlUI::SetPosition(lpRect);
	...
	for( int it = 0; it < m_items.GetSize(); it++ ) {
		...
		if( pControl->IsFloat() ) {
			...
			pControl->SetPosition(&rc); // 所有非float子控件放大到整个客户区
		}
	}
}
...
LRESULT CRichEditUI::MessageFilter(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	...
	if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
		...
		if( dwHitResult == HITRESULT_OUTSIDE ) {
			...
			POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
			if( uMsg == WM_MOUSEWHEEL ) ::ScreenToClient(GetManager()->GetHWND(), &pt);
		...
		POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
		::ScreenToClient(GetManager()->GetHWND(), &pt);
		CControlUI* pHover = GetManager()->FindControl(pt);
		if(pHover != this) {
			bWasHandled = false;
			return 0;
...
void CRichEditUI::Render(IUIRender* pRender,LPCRECT prcTemp)
{
	RECT rcTemp = { 0 };
	if( !::IntersectRect(&rcTemp, prcTemp, &m_rcControl) )
		return;
	
	CRenderClip clip;
	CRenderClip::GenerateClip(pRender->GetPaintDC(), rcTemp, clip);
	CControlUI::Render(pRender, prcTemp);
	...
	if( m_pTwh )
	{
		...
		m_pTwh->GetTextServices()->TxDraw(
			...
			pRender->GetPaintDC(),			        // Draw device HDC
			NULL, 				   	// Target device HDC
			(RECTL*)&rc,			// Bounding client rectangle
			NULL, 		            // Clipping rectangle for metafiles
			(RECT*)&rcTemp,		// Update rectangle
	...
	if( m_items.GetSize() > 0 ) {
		...
		if( !::IntersectRect(&rcTemp, &rcTemp, &rc) ) {
			for( int it = 0; it < m_items.GetSize(); it++ ) {
				CControlUI* pControl = static_cast<CControlUI*>(m_items[it]);
				if( !pControl->IsVisible() ) continue;
				if( !::IntersectRect(&rcTemp, &rcTemp, &pControl->GetPosition()) ) continue;
				if( pControl ->IsFloat() ) {
					if( !::IntersectRect(&rcTemp, &m_rcControl, &pControl->GetPosition()) ) continue;
					pControl->Render(pRender, &rcTemp);
				}
			}
		}
		else {
	...
	if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() ) {
		if( ::IntersectRect(&rcTemp, &rcTemp, &m_pVerticalScrollBar->GetPosition()) ) {
			m_pVerticalScrollBar->Render(pRender, &rcTemp);
		}
	}

	if( m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsVisible() ) {
		if( ::IntersectRect(&rcTemp, &rcTemp, &m_pHorizontalScrollBar->GetPosition()) ) {
			m_pHorizontalScrollBar->Render(pRender, &rcTemp);
		}
	}
}

//////////////////////////////////////////////////////////////////////////
// CTxtWinHost Implement
...
{
	return ::CreateCaret(m_re->GetManager()->GetHWND(), hbmp, xWidth, yHeight);
}

BOOL CTxtWinHost::TxShowCaret(BOOL fShow)
{
	if(fShow)
		return ::ShowCaret(m_re->GetManager()->GetHWND());
	else
		return ::HideCaret(m_re->GetManager()->GetHWND());
}
...
{
	return ::ScreenToClient(m_re->GetManager()->GetHWND(), lppt);	
}

BOOL CTxtWinHost::TxClientToScreen(LPPOINT lppt)
{
	return ::ClientToScreen(m_re->GetManager()->GetHWND(), lppt);
}

...
namespace DuiLib {
	...
	{
		BOOL hasBorderSize = ((m_rcBorderSize.left > 0) || (m_rcBorderSize.bottom > 0) || (m_rcBorderSize.top > 0) || (m_rcBorderSize.right > 0));
		BOOL needPaint = (hasBorderSize && (m_dwBorderColor != 0 || m_dwFocusBorderColor != 0));

		if (needPaint) {
			BOOL hasBorderRound = (m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0);
			if (hasBorderRound)//画圆角边框
			{
